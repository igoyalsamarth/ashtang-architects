  ...
  return (
    <main className={`flex w-[100vw] h-[100vh] max-w-[100vw] max-h-[100vh] min-w-[100vw] min-h-[100vh] relative text-white duration-200 ${isNavbarOpen ? 'py-4 pr-4' : ''}`}>
      <div className="flex w-full h-full">
        <div className={`fixed top-0 left-0 h-full text-[#0c1d1c] transition-all duration-200 ease-in-out z-20 ${isNavbarOpen ? 'w-64' : 'w-0'}`}>
          <div className={`flex flex-col overflow-hidden duration-200 ${isNavbarOpen ? 'p-4' : ''}`}>
            <Link href='/about' className="px-3 py-2 hover:bg-[#dffffe] rounded duration-200">About</Link>
            <Link href="/projects" className="px-3 py-2 hover:bg-[#dffffe] rounded duration-200">Projects</Link>
            <Link href="/contact" className="px-3 py-2 hover:bg-[#dffffe] rounded duration-200">Contact</Link>
          </div>
        </div>
        {!isNavbarOpen && <button
          className="fixed top-4 left-4 w-10 h-10 rounded-full bg-black z-30 flex items-center justify-center"
          onClick={() => setIsNavbarOpen(!isNavbarOpen)}
        >
          <Image src="/logo.png" alt="☰" width={20} height={20} className="text-white" />
        </button>}
        <div onClick={() => setIsNavbarOpen(false)} className={`relative flex w-full h-full duration-200 ${isNavbarOpen ? 'ml-64 rounded-xl overflow-hidden' : ''}`}>
          <video
            autoPlay
            loop
            muted
